...
def execute(gid):
    ...
    # すでにSheetに記載済みのアカウントは判定対象外としたいのでリストから除く。
    target_ids = [f_id for f_id in follower_ids if f_id not in user_ids]
...
def evaluate_candidate(api, job_words_dict, career_words_dict, user_ids, users):
    ...
    for index, u in enumerate(users):
        ...
        try:
            if u.id in user_ids or u.protected or u.followers_count >= 20000 or is_antisocial([u.description]):
                print('判定対象外にしました。（Sheet記載済み or 鍵アカウント or フォロワー数が多すぎ(2万以上) or プロフィールに反社会的な単語が含まる。）')
                continue
